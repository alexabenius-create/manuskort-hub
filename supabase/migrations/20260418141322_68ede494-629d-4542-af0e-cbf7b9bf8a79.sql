-- Atomär import av manus + paneldeltagare + kort i en transaktion.
-- Klienten skickar JSONB-payloads och får tillbaka nya manuscript_id.
-- Talare i kortens HTML refereras med "tempId" (t.ex. "tmp:Alexander") och
-- byts ut mot riktiga panelist-id:n efter att paneldeltagarna skapats.

create or replace function public.import_manuscript(
  p_manuscript jsonb,
  p_panelists jsonb,
  p_cards jsonb
)
...
begin
  ...
  -- 2) Skapa paneldeltagare och bygg map { tempId -> nytt id }
  v_pos := 0;
  ...
  -- 3) Skapa kort (ersätt tempId i HTML mot riktiga panelist-id)
  v_pos := 0;
  for v_card in select * from jsonb_array_elements(coalesce(p_cards,'[]'::jsonb))
  loop
    v_html := coalesce(v_card->>'content_html','');
    -- Byt ut alla data-panelist-id="tmp:..." mot verkliga id
